...
const updateTasksInLocalStorage = async (UID) => {
  try {
    ...
    if (!usersSnapshot.empty) {
      ...
      // Ensure 'dueDate' exists and is a Timestamp
      ...
      const allTasks = allTasksSnapshot.docs
        .filter(taskDoc => !userTasks.some(userTask => userTask.id === taskDoc.id) && taskDoc.data().status !== 'under-review')
        .map(taskDoc => {
          ...
          {loading ? (
            ...
            <>
              ...
              >
                ...
                <StatBox icon={<FaRegClock size="2em" />} title="Completed Tasks" value={pendingApprovalTasks} color="#fcd4c8" />
              ...
              >
                <TaskStatus 
                  ...
                  pendingApproval={0} 
                />
                {loading ? <p>Loading tasks...</p> : <TaskTable tasks={tasks} onStartTask={handleStartTask} page={5} />}
              </div>
            </>
          )}
        </div>
